...
| 能力 | 当前位置 | 保留原因 |
| --- | --- | --- |
| Agent-independent CLI | `src/adapters/standalone/` | 不绑定具体 AI 开发代理宿主，符合项目边界。 |
...
| 代码和配置文件 | verified | 会保护代码、参数、链接字符串，不应被普通正文清洗误伤。 |
...
| 最高流程阶段 | 当前状态 | 结论 |
| --- | --- | --- |
| 用户给资料、入口接收 | 基本存在 | CLI 输入和批量目录扫描已有；后续重点是本地图片、音视频和复杂文档入口。 |
| 识别来源和格式 | 部分存在 | 本地文件扩展名、PDF/Office 基础诊断已有；本地导出文件、批次和文件夹来源说明还弱。 |
...
1. 保留 agent-independent 边界，不做任何宿主专属业务逻辑。
...
1. 转换路线注册表：把 MinerU、Docling、Pandoc、ASR 等本地文件处理路线接成可评估的 route，而不是散落在代码里。
...
2. 更新 `docs/known-issues.md`：把图片 OCR、旧 Office/MOBI、音视频、本地 HTML/导出文件列为明确路线而非模糊 backlog。
3. 更新核心流程文档和 CLI 使用说明：让使用者知道哪些输入可直接跑，哪些必须先预处理。
...
参考：MinerU README 说明其支持 PDF、图片、DOCX、PPTX、XLSX 到 Markdown/JSON，并支持扫描件、表格、公式、109 语言 OCR；Docling README 说明其可导出 Markdown、HTML、WebVTT、lossless JSON，并支持 OCR、图片、ASR、本地运行。

### 7.2 MarkItDown 只做架构借鉴

MarkItDown 对 KBPrep 的价值是产品和工程设计参考，不是新增依赖，也不是转换工具选型结论：

- 它把输入识别、格式判断、converter 选择和转换结果拆成清晰边界，KBPrep 可以借鉴这种模块边界。
- 它允许不同格式由不同 converter 处理，KBPrep 可以借鉴为 route registry，但每条 route 仍必须进入 KBPrep 的质量门。
- 它把重依赖放在可选能力里，KBPrep 后续接 OCR、ASR 或复杂 Office 路线时也应保持可选，避免拖慢普通本地文档处理。

本项目只借鉴这种设计方法，不增加 MarkItDown 依赖，也不改变默认转换路线。
...
3. **本地 HTML/导出文件质量加固**
   - 继续处理已经保存到本地的 HTML 或导出文本。
   - 重点验证标题、正文、列表、表格、图片引用和正文链接是否保留。
   - 不新增远程地址获取能力。
...
2. **faster-whisper 或 whisper.cpp ASR route**
   - 用于无字幕音视频。
   - 只处理用户已经获取到本地的音视频文件。
...
### 阶段 3：本地 HTML/导出文件质量加固

目标：把已经保存到本地的 HTML 或导出文件，从“粗读文本”升级为“结构保留 + 来源证据”。

任务：

1. 继续加固本地 HTML route。
2. 输出 metadata：文件路径、标题、检测到的结构、转换工具。
3. 失败时保留原文件和错误说明。
...
- 有本地字幕或转写文件：进入现有字幕/转写流程。
- 无字幕的本地音视频文件：faster-whisper 或 whisper.cpp 生成转写，再进入字幕/转写流程。
...
2. 不要让 Docling、MinerU、Pandoc 任何一个工具直接绕过质量门。
3. 不要默认接入所有重依赖，否则本地安装和维护会变复杂。
4. 不要把远程地址处理、站点内容获取或线上音视频获取做成产品能力；音视频转写也必须只从用户已获取的本地文件出发。
...
下一步建议从 **路线注册表 + 转换对比报告 + 黄金样本库 + Docling candidate route** 开始。这样既能快速提升真实资料处理能力，又不会破坏现有可演示闭环。
