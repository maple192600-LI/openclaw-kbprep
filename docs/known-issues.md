...
- `#2`: AI review now routes through a reusable backend abstraction. Provider-specific clients are not maintained in this repository; callers may inject their own review backend.
- `#3`: `python/kbprep_worker/prepare.py` is now a thin orchestrator entrypoint with the heavy pipeline moved into stage modules.
- `#4`: The old generated docs website was removed from the maintained docs surface. Current docs stay focused on the core flow, implementation plan, capability matrix, quality loop, feedback learning, and CLI usage.
...
- Standalone AI review does not include a vendor-specific LLM client. It supports caller-injected backends and an agent-independent external command protocol; when no backend is configured it returns a warning instead of pretending AI review ran.
...
The project package is `kbprep`. This repository provides CLI commands, rule dictionaries, and core workflow documentation. Agent-host packaging should be generated outside the maintained core.

## Maintained Docs Surface

The maintained docs set is intentionally narrow:
...
Old generated showcase pages, historical Superpowers plans, internal map reports, and duplicate architecture summaries are intentionally not part of the maintained docs surface.
