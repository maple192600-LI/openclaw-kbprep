...
def publish_latest_outputs(run_dir: Path, root_p: Path, input_p: Path, profile: str = "standard") -> dict:
    ...
    if dst_parts.exists():
        safe_rmtree(dst_parts, root=root_p)
    if src_parts.exists():
        shutil.copytree(src_parts, dst_parts)
    else:
        dst_parts.mkdir(parents=True, exist_ok=True)

    src_quality_gates = run_dir / "quality_gates"
    dst_quality_gates = root_p / "quality_gates"
    if dst_quality_gates.exists():
        safe_rmtree(dst_quality_gates, root=root_p)
    if src_quality_gates.exists():
        shutil.copytree(src_quality_gates, dst_quality_gates)

    src_images = run_dir / "images"
    dst_images = root_p / "images"
    if dst_images.exists():
        safe_rmtree(dst_images, root=root_p)
    ...
    if dst_obsidian.exists():
        safe_rmtree(dst_obsidian, root=root_p)
